...
import { getApplications, verifyByState, updateApplicationStatus } from "../store";
...
export default function DNODashboard({ admin, onLogout }) {
  ...
  const awaiting = apps.filter(a => a.status === "Institute Verified" && match(a));
  const approved = apps.filter(a => a.status === "State Verified" && match(a));
  const rejected = apps.filter(a => a.status === "Rejected" && match(a));
  const all      = apps.filter(match);
  ...
  const stats = {
    total:    apps.length,
    awaiting: apps.filter(a => a.status === "Institute Verified").length,
    approved: apps.filter(a => a.status === "State Verified").length,
    rejected: apps.filter(a => a.status === "Rejected").length,
  };

  const handleApprove = async (appId) => {
    try {
      await verifyByState(appId, "state");
      await loadApps();
      showToast("✅ Application verified by State & forwarded to Ministry Officer!");
    } catch (err) {
